...
function LoadMenuSystem(){
	...
	while ( $xMenu1 -lt 1 -or $xMenu1 -gt 4 ){
		...
		if( $xMenu1 -lt 1 -or $xMenu1 -gt 4 ){
			Write-Host "`tPlease select one of the options available.`n" -Fore Red;start-Sleep -Seconds 3
		}
	}
		...
		1 {
			...
			Switch ($xMenu2){
				1{      clear
                        ...
                        Write-Host You wrote $FullName, with a username of $Username.ToLower() reports to $ReportsTo in Company $CompanyNumber
                        }
